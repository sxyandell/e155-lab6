...
void initSPI(int br, int cpol, int cpha) {
    ...
    // Set output speed type to high for SCK (PB3)
    GPIOB->OSPEEDR |= (GPIO_OSPEEDR_OSPEED3);

    // Set to AF05 for SPI alternate functions
    GPIOB->AFR[0] |= _VAL2FLD(GPIO_AFRL_AFSEL3, 5); // PB3 SCK AF5
    ...
    // Use software NSS management to avoid external NSS dependency
